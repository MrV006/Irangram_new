...
export const adminSendMessageAsUser = async (chatId: string, spoofedSenderId: string, text: string, replyToId?: string) => { if (!db || !text.trim()) return; try { const userProfile = await getUserProfile(spoofedSenderId); const messageData: any = { text, senderId: spoofedSenderId, senderName: userProfile?.name || 'User', senderAvatar: userProfile?.avatar || '', type: 'text', createdAt: serverTimestamp(), isSticker: false, reactions: {} }; if(replyToId) messageData.replyToId = replyToId; if (chatId === 'global_chat') { await addDoc(collection(db, "global_chat"), messageData); } else { await addDoc(collection(db, "chats", chatId, "messages"), messageData); await setDoc(doc(db, "chats", chatId), { lastMessage: text, lastSenderId: spoofedSenderId, updatedAt: serverTimestamp() }, { merge: true }); } } catch(e) {} };
export const adminEditUserMessage = async (chatId: string, messageId: string, newText: string) => { if (!db) return; try { if (chatId === 'global_chat') { await updateDoc(doc(db, "global_chat", messageId), { text: newText, edited: true }); } else { await updateDoc(doc(db, "chats", chatId, "messages", messageId), { text: newText, edited: true }); } } catch(e) { console.error("Admin edit failed", e); } };
